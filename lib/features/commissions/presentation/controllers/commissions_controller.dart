...
class CommissionsController extends GetxController {
  final CommissionsUsecase commissionsUsecase;

  CommissionsController(this.commissionsUsecase);
 ...
 */
   ...
   * @Setter
   */
  final _items = <CommissionModel>[].obs;

  /**
   * List of products 
   * @ Getter
   */
  List<CommissionModel> get commissions => _items.toList();
  ...
  Future<void> payAllCommissions(context) async {
    ...
    final failureOrPayCommissions =
        await commissionsUsecase.payAllCommissions(paymentMethod);
...
_isLoadingList.value = true;
    ...
    final failureOrCategories =
        await commissionsUsecase(_paginationFilter.value);
